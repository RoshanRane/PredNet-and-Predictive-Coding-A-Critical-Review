...
from prednet_data_utils import SmthSmthSequenceGenerator
...
if args.train_model_flag:
    ...
    history = model.fit_generator(train_generator, args.sample_size / args.train_batch_size, args.nb_epoch, callbacks=callbacks,
                                  validation_data=val_generator, validation_steps=args.n_seq_val/args.train_batch_size)
    if save_model:
...
if args.evaluate_model_flag:
    ...
    weights_file = os.path.join(args.weight_dir, 'tensorflow_weights/prednet_weights.hdf5')
    json_file = os.path.join(args.weight_dir, 'prednet_model.json')
    ...
    test_generator  = SmthSmthSequenceGenerator(data_csv, "test"
                                                , nframes=args.nframes
    ...
    f = open(args.result_dir + 'prediction_scores.txt', 'w')
    f.write("Model MSE: %f\n" % mse_model)
